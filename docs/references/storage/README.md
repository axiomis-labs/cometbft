...
All the experiments were performed on `main` after `v1-alpha.1` was released. The experiments on Axiom were done on custom branches porting the changes to `0.37.x` with the changes Axiom has on their fork of 0.37:
- [Axiom testing "v1"](https://github.com/cometbft/cometbft/tree/storage/tmp/axiom/v0.37.x-testing-validator)
- [Axiom testing "v2"](https://github.com/cometbft/cometbft/tree/storage/tmp/axiom/v0.37.x-testing-newlayout-validator)

We also have non Axiom specific backports to 0.37.x based code in the following branches:
...
The one experiment with a real application (axiom) did not back up this theory though. Even though the overall performance was better than the software version used by the application at the moment.
...
- we confirmed that moving pruning + compaction to background mitigates potential performance impact of this feature, by running tests with it on Axiom mainnet
...
- regarding key layouts:
  ...
  - we expect to continue working with operator teams to gather data from production, ideally Axiom and Osmosis
...
3. **production-testing**: The validator team at Informal Staking was kind enough to spend a lot of time with us trying to evaluate our changes on full nodes running on mainnet Axiom chains. As their time was limited and they had found initially that pruning, in addition to not working, slows down Axiom nodes, we were interested to understand the impact our changes made on their network. Future investigation on mainnet nodes would be required to gather more real-world data on chains with different demands and see if pruning is indeed ineffective and the slow down is reproducible.
...
### Production - Axiom mainnet

#### Pruning without compaction

![axiom-no-compaction](img/axiom_no_compaction.png "Axiom -  pruning without compaction")
#### Pruning with compaction

![axiom-compaction](img/axiom_compaction.png "Axiom - pruning with compaction")
...
### Production - Axiom mainnet
The impact of this changes is best demonstrated with the runs by Informal staking comparing 4 Axiom nodes with the following setup:

1. *axiom-sentry0* comet="v0.37" , pruning="default", keylayout=old
2. *axiom-sentry1* comet="v0.37" , pruning="none" , keylayout=old
3. *axiom-pruning* comet="modified" , pruning="600blocks" , keylayout=old
4. *axiom-newlayout* comet="modified" , pruning="600blocks" , keylayout=new
...
![axiom-commit](img/axiom_commit.png "Axiom - commit")
...
![axiom-bpt](img/axiom_block_processing_time.png "Axiom - average block processing time")
...
When backporting these changes to the 0.37.x based branch we gave to Informal staking, we obtained similar results when ran locally on the kvstore app. However, this is not what they observed on mainnet. In the graphs above, we see that the new layout, while still improving performance compared to CometBFT v0.37.x, introduced a ~10ms latency in this particular case. According to the operators, this is a big difference for chains like Axiom.
...
Without pruning, for a bigger database and block processing times (on runs with our e2e), the new layout lowered the overall block processing time even without pruning. When the block times are very low (in our local setup or on Axiom), we did not observe the same benefits.
